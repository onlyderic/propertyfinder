...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null, $photo_folder_id = '') {
        $this->options = array(
            ...
            'image_versions' => array(
                ...
                */
                'thumbs' => array(
                    'max_width' => PROPPROFILEPIC_MAXWIDTH,
                    'max_height' => PROPPROFILEPIC_MAXHEIGHT
                )
            ),
    ...
    protected function get_user_id() {
        @session_start();
        return $this->options['photo_folder_id'];
    }
    ...
    protected function set_file_delete_properties($file) {
        $file->delete_url = $this->options['script_url'].'remove/'
            .$this->get_query_separator($this->options['script_url'])
    ...
    protected function get_file_size($file_path, $clear_stat_cache = false) {
        if ($clear_stat_cache) {
            clearstatcache(); //clearstatcache(true, $file_path);
        }
    ...
    public function post($print_response = true) {
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            foreach ($upload['tmp_name'] as $index => $value) {
                $path_parts = pathinfo($upload['name'][$index]);
                $file_name = md5( date('YmdHis') . '_' . $path_parts['filename'] ) . '.' . $path_parts['extension'];
    ...
    public function post2($print_response = true) {
        if (isset($_REQUEST['_method']) && $_REQUEST['_method'] === 'DELETE') {
            return $this->delete($print_response);
        }
        $upload = isset($_FILES[$this->options['param_name']]) ?
        ...
        if ($upload && is_array($upload['tmp_name'])) {
            ...
            // param_name is a single object identifier like "file",
            // $_FILES is a one-dimensional array:
            $path_parts = pathinfo($upload['name']);
            $file_name = md5( date('YmdHis') . '_' . $path_parts['filename'] ) . '.' . $path_parts['extension'];
            $this->new_file_name = $file_name;
            $files[] = $this->handle_file_upload(
                isset($upload['tmp_name']) ? $upload['tmp_name'] : null,
