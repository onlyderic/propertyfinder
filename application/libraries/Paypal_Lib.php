...
class Paypal_Lib {
	...
	function Paypal_Lib()
	{
		...
		$paymentmode = $this->CI->config->item('paymentmode');
	...
	function validate_ipn()
	{
		...
		// script.
		$post_string = '';	 
		if (isset($_POST)) //if ($this->CI->input->post())
		{
			foreach ($_POST as $field=>$value) //foreach ($this->CI->input->post() as $field=>$value)
			{
                                $value = str_replace("\n", "\r\n", $value); //Added
		...
		if (eregi("/VERIFIED/i",$this->ipn_response) || $this->paypal_url == 'https://www.sandbox.paypal.com/cgi-bin/webscr')
		{
